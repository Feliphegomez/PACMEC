...
function pacmec_head()
{
  ...
  do_action( "head" );
  $a = "";
  foreach($GLOBALS['PACMEC']['website']['styles']['head'] as $file){ $a .= \PACMEC\Util\Html::tag($file['tag'], "", [], $file['attrs'], true)."\t"; }
  $a .= \PACMEC\Util\Html::tag('style', do_action( "head-styles" ), [], ["rel"=>"stylesheet", "type"=>"text/css", "charset"=>"UTF-8"], false) . "\t";
  foreach($GLOBALS['PACMEC']['website']['scripts']['head'] as $file){ $a .= \PACMEC\Util\Html::tag($file['tag'], "", [], $file['attrs'], false)."\t"; }
...
function pacmec_foot()
{
  ...
  $a = "";
	foreach($GLOBALS['PACMEC']['website']['styles']['foot'] as $file){ $a .= \PACMEC\Util\Html::tag($file['tag'], "", [], $file['attrs'], true)."\t"; }
  $a .= \PACMEC\Util\Html::tag('style', do_action( "footer-styles" ), [], ["rel"=>"stylesheet", "type"=>"text/css", "charset"=>"UTF-8"], false) . "\t";
	foreach($GLOBALS['PACMEC']['website']['scripts']['foot'] as $file){ $a .= \PACMEC\Util\Html::tag($file['tag'], "", [], $file['attrs'], false)."\t"; }
...
function type_options($key) : Array
{
  ...
  if(!isset($PACMEC['types_options'][$key])){
    $PACMEC['types_options'][$key] = [];
    foreach($PACMEC['DB']->FetchAllObject("SELECT * FROM `{$PACMEC['DB']->getTableName('types_options')}` WHERE `type` IN (?) ORDER BY `name` ASC", [$key]) as $option){
      $option->name = ($option->autot==1) ? __a($option->name) : $option->name;
      $PACMEC['types_options'][$key][] = $option;
    }
  }
  return $PACMEC['types_options'][$key];
}
