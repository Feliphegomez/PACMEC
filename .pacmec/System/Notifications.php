...
class Notifications extends \PACMEC\System\BaseRecords
{
  ...
  public static function table_list_html($items) : String
  {
    ...
    foreach ($items as $uid => $item) {
      switch ($item->is_read) {
        case 1:
          $icon = \PACMEC\Util\Html::tag('a',
            \PACMEC\Util\Html::tag('i', '', ['fa fa-check-circle'], ["id"=>'pacmec-change-status-notification-fast-icon-'.$item->id])
          , ['pacmec-change-status-notification-fast'], ['href'=>'#', 'data-notification_id'=>$item->id]);
          break;
        case 0:
        $icon = \PACMEC\Util\Html::tag('a',
          \PACMEC\Util\Html::tag('i', '', ['fa fa-dot-circle-o'], ["id"=>'pacmec-change-status-notification-fast-icon-'.$item->id])
        , ['pacmec-change-status-notification-fast'], ['href'=>'#', 'data-notification_id'=>$item->id]);
          break;
        default:
          $icon = \PACMEC\Util\Html::tag('a',
            \PACMEC\Util\Html::tag('i', '', ['fa fa-circle-o'], ["id"=>'pacmec-change-status-notification-fast-icon-'.$item->id])
          , ['pacmec-change-status-notification-fast'], ['href'=>'#', 'data-notification_id'=>$item->id]);
          break;
      }
      $table->addRow([
        \PACMEC\Util\Html::tag('a',
          (!empty($item->title) ? (\PACMEC\Util\Html::tag('b', $item->title)."<br />") : "")
          . $item->message
      ...
      /*
        ...
        , ($order->pay_enabled==true)
            ? \PACMEC\Util\Html::tag('a', \PACMEC\Util\Html::tag('i', '', ['fa fa-usd']) . ' Pagar ahora ', ['btn btn-sm btn-outline-primary btn-hover-primary'], ['href'=>$order->link_pay])
            : ($order->payment_date!== null ? \time_passed($order->payment_date) : '-')
        // , \PACMEC\Util\Html::tag('a', 'View', ['btn btn-sm btn-outline-dark btn-hover-primary'], ['href'=>$order->link_view])
        // , \time_passed(date('m-d-Y H:i:s', strtotime($order->created)))
        //, \PACMEC\Util\Html::tag('a', \PACMEC\Util\Html::tag('i', '', ['fa fa-comment']), ['btn btn-sm btn-outline-secondary btn-hover-dark'], ['href'=>$order->link_view])
      */
      ]);
    }
    return $table;
