...
class Products extends \PACMEC\System\ModeloBase {
  ...
  const COLUMNS_AUTO_T  = [];
	...
	public function getBy($column='id', $val=""){
		try {
			return $this->setAll($GLOBALS['PACMEC']['DB']->FetchObject("SELECT * FROM `{$GLOBALS['PACMEC']['DB']->getTableName(SELF::MODEL_TBL)}` WHERE `{$column}`=? AND `host` IN ('*', ?)", [$val, $GLOBALS['PACMEC']['host']]));
		}
		catch(Exception $e){
			return $r;
		}
  ...
  {
    $add_sql = " WHERE `is_active` IN (1) AND `host` IN ('*', ?) ORDER BY `created` DESC";
    $sql = "SELECT `id` FROM `{$GLOBALS['PACMEC']['DB']->getTableName(SELF::MODEL_TBL)}` WHERE `id` IN (?) ".$add_sql;
    $item = $GLOBALS['PACMEC']['DB']->FetchObject($sql, [$id, $GLOBALS['PACMEC']['host']]);
  ...
  {
    $sql = "SELECT MIN(`price_normal`) AS `price_min`, MAX(`price_normal`) AS `price_max` FROM `{$GLOBALS['PACMEC']['DB']->getTableName(SELF::MODEL_TBL)}` WHERE `host` IN ('*', ?) ";
  ...
  public static function allNews($limit=null) : array
  {
    ...
    $add_sql = " WHERE `is_active` IN (1) {$in_gallery_sql_p_a} AND `host` IN ('*', ?) ORDER BY `created` DESC";
    $r = [];
    if($limit==null){
    ...
    foreach($GLOBALS['PACMEC']['DB']->FetchAllObject($sql, [$GLOBALS['PACMEC']['host']]) as $item){
