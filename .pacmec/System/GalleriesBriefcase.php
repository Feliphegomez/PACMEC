...
class GalleriesBriefcase extends \PACMEC\System\BaseRecords
{
  const TABLE_NAME = 'services_pictures';
  ...
  {
    $folder_galleries_briefcase = dirname(PACMEC_PATH) . "/public/galleries/briefcase";
    $folder_public = dirname(PACMEC_PATH);
    $r = [];
    foreach(\glob("{$folder_galleries_briefcase}/*") as $file_path){
      $ref_sku  = basename($file_path);
      if(!isset($r[$ref_sku])) $r[$ref_sku] = [];
      $service = new \PACMEC\System\Service((object) ["ref"=>$ref_sku]);
      ...
      if($service->isValid()){
        foreach ($service->gallery as $path_short) {
          if($path_short !== infosite('default_picture')){
            if(isset($r[$ref_sku][$path_short])){
              $r[$ref_sku][$path_short]['in_service'] = true;
            } else {
              $r[$ref_sku][$path_short] = [
                'in_folder'   => false,
                'in_service'  => true,
  ...
  public static function table_list_html(array $items) : String
  {
    ...
    $table->addHeaderRow([
      \PHPStrap\Util\Html::tag('a', \PHPStrap\Util\Html::tag('i', '', ['fa fa-refresh']), ['btn btn-sm btn-outline-success btn-hover-success'], [
        'href'=>__url_s("/%admin_galleries_briefcase_slug%?reload=true")
      ])
      , __a('sku_ref')
      , __a('name')
      , __a('common_names')
    ...
    foreach ($items as $sku => $item) {
      ...
      */
      foreach ($item as $img) {
        $btn_in_folder = "";
        $btn_in_service = "";
        if($img['in_folder'] == true && $img['in_service'] == true && $img['service'] > 0) $btn_in_folder .= \PHPStrap\Util\Html::tag('a', \PHPStrap\Util\Html::tag('i', '', ['fa fa-check']), ['pacmec-button pacmec-padding-small pacmec-circle pacmec-white pacmec-border pacmec-border-teal'], [ 'href'=>$img['link_view'] ]);
        //if($img['in_folder'] == true && $img['in_service'] == false && $img['service'] > 0) $btn_in_folder .= \PHPStrap\Util\Html::tag('a', \PHPStrap\Util\Html::tag('i', '', ['fa fa-plus']), ['pacmec-button pacmec-padding-small pacmec-circle pacmec-white pacmec-border pacmec-border-green'], [ 'href'=>"#" ]);
        #if($img['service'] <= 0) $btn_in_folder .= \PHPStrap\Util\Html::tag('a', \PHPStrap\Util\Html::tag('i', '', ['fa fa-times']), ['pacmec-button pacmec-padding-small pacmec-circle pacmec-white pacmec-border pacmec-border-red'], [ 'href'=>"#" ]);
        $table->addRow([
  ...
  public static function reload_all_galleries_briefcase()
  {
    $html = "";
    $html .= \PACMEC\Util\Html::tag('p', "*** *** *** Start *** *** ***", [], []);
    foreach (\PACMEC\System\GalleriesBriefcase::get_all_in_gallery() as $sku => $items) {
      ...
      foreach ($items as $item) {
        ...
        if ($item['service'] > 0 && $item['in_service'] == true && $item['in_folder'] == true) {
          $html .= \PACMEC\Util\Html::tag('p', "Existente.", [], []);
          $html .= \PACMEC\Util\Html::tag('i', "", ['fa fa-check-square-o'], []);
        }
        elseif ($item['service'] > 0 && $item['in_service'] == true) {

        }
        elseif ($item['service'] > 0 && $item['in_service'] == false) {
          $create = new Self();
        ...
        elseif (!($item['service'] > 0)) {
          $html .= \PACMEC\Util\Html::tag('p', "Serviceo no existe.", [], []);
          $html .= \PACMEC\Util\Html::tag('i', "", ['fa fa-times'], []);
        }
        $i++;
      }
