      ...
      switch ($GLOBALS['PACMEC']['fullData']['type']) {
        case 'product':
           if(isset($GLOBALS['PACMEC']['fullData']['product_id'])){
             ...
             echo $msg_add_product = \PACMEC\Util\Html::tag('div', __a($result), ['alert alert-success']);
        ...
        case 'service':
           if(isset($GLOBALS['PACMEC']['fullData']['service_id'])){
             ...
             $url_cart = __url_s('/%cart_slug%');
             echo $msg_add_service = \PACMEC\Util\Html::tag('div', __a($result), ['alert alert-success']);
             //echo "<meta http-equiv=\"refresh\" content=\"0;\" />";
             //header("Refresh:0");
           }
          break;
        ...
        if($siteObj !== false){
          ...
          if($orders_statuses !== false){
            foreach ($orders_statuses as $orders_status) {
              ...
              $status->total = 0;
              $count = $GLOBALS['PACMEC']['DB']->FetchObject("SELECT COUNT(`O`.`id`) as 'count' FROM `{$GLOBALS['PACMEC']['DB']->getTableName('orders')}` `O` WHERE `O`.`host` IN (?) AND `O`.`status` IN (?)", [$siteObj->domain, $orders_status->id]);
              if($count !== false) $status->total = $count->count;
              $siteObj->orders_status[] = $status;
            }
          }

          $orders = $GLOBALS['PACMEC']['DB']->FetchAllObject("SELECT `O`.`id` FROM `{$GLOBALS['PACMEC']['DB']->getTableName('orders')}` `O` WHERE `O`.`host` IN (?)", [$siteObj->domain]);
          if($orders !== false){
            foreach ($orders as $order) {
              $siteObj->orders[] = $order->id;
            }
          }
